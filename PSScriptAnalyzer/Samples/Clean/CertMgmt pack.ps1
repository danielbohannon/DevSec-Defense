...
[CmdletBinding()]
	...
	# определение, что для PFX сертификатов указан пароль
	...
	# конвертация предварительных значений аргументов в конечные значения и типы
	# .NET классов
	...
	"Pkcs7|SerializedStore" {
			# создаём объект массива x509Certificate2 объектов
			$certcol = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2Collection
			# добавляем все сертификаты из $certs в этот массив
			$certs | %{[void]$certcol.Add($_)}
			# экспортируем этот массив в байтовый массив
			$bytes = $certcol.Export($Type)
			# записываем массив байтов в p7b файл
